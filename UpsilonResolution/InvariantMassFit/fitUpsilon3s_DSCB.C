...
void fitUpsilon3s_DSCB(const char* infileName = "../Files/Q1_Upsilon3SMass.root"){
   ...
   // Y(1S) signal shape
	RooRealVar mean_1S("mean_1S", "mean 1S", PDGmass_3S, 9.9, 10.5);
	RooRealVar sigma_1S("sigma_1S", "",0.08, .01, .15);

   // get the tail parameters of the signal shape first in case the MC fit is needed
	RooRealVar* alphaInf = new RooRealVar("alphaInf", "", 1.5, 0, 10);
	RooRealVar* orderInf = new RooRealVar("orderInf", "", 1.5, 0, 10);
	RooRealVar* alphaSup = new RooRealVar("alphaSup", "", 1.5, 0, 10);
	RooRealVar* orderSup = new RooRealVar("orderSup", "", 3.,  0, 10);
	...
	RooRealVar nCB1S("nCB1S", "number of signal CB 1S", 5.e5, 3e5, 5.5e5); //450
